...
class TestGetLogger:
	def test_returns_appwrite_logger_when_context_present(self):
		...
		with patch("src.adapters.rest.fastapi_routes.AppwriteLogger") as mock_appwrite:
	...
	def test_returns_default_logger_when_no_context(self):
		...
		with patch("src.adapters.rest.fastapi_routes.DefaultLogger") as mock_default:
...
class TestUserRoutes:
	def test_get_or_create_user_by_identity_calls_handler(self):
		...
		with patch("src.adapters.rest.fastapi_routes.UserIdentityHandler") as mock_handler:
		...
		logger.info.assert_called_with(
			"User identity: google_123 for provider: google"
		)
	...
	def test_get_or_create_user_by_identity_allows_none_email(self):
		...
		with patch("src.adapters.rest.fastapi_routes.UserIdentityHandler") as mock_handler:
			...
			run_async(fastapi_routes.get_or_create_user_by_identity(request, logger=logger))    
...
class TestReceiptRoutes:
	def test_get_or_create_receipt_calls_handler(self):
		...
		with patch("src.adapters.rest.fastapi_routes.SfsMdReceiptHandler") as mock_handler:
			mock_handler.return_value.get_or_create.return_value = expected_response

			result = run_async(fastapi_routes.get_or_create_receipt(request, logger=logger))    
	...
	def test_get_or_create_receipt_with_multiple_purchases(self):
		...
		receipt.purchases = [
			PurchasedItem(name="Item A", quantity=2.0, unit=QuantityUnit.PIECE, price=6.17),
			PurchasedItem(name="Item B", quantity=1.5, unit=QuantityUnit.KILOGRAM, price=10.50),
			PurchasedItem(name="Item C", quantity=3.0, unit=QuantityUnit.PIECE, price=5.00),
		]

		with patch("src.adapters.rest.fastapi_routes.SfsMdReceiptHandler") as mock_handler:
			mock_handler.return_value.get_or_create.return_value = receipt

			result = run_async(fastapi_routes.get_or_create_receipt(receipt, logger=logger))
	...
	def test_get_or_create_receipt_with_single_purchase(self):
		...
		receipt = make_receipt()
		receipt.purchases = [
			PurchasedItem(name="Single Item", quantity=1.0, unit=QuantityUnit.PIECE, price=5.00),
		]
	...
	def test_get_or_create_receipt_with_many_purchases(self):
		...
		receipt = make_receipt()
		receipt.purchases = [
			PurchasedItem(name=f"Item {i}", quantity=float(i), unit=QuantityUnit.PIECE, price=float(i))
	...
	def test_get_receipt_by_url_handler_exception(self):
		...
		with patch("src.adapters.rest.fastapi_routes.SfsMdReceiptHandler") as mock_handler:
			mock_handler.return_value.get_by_url.side_effect = Exception("Database connection error")
	...
	def test_get_or_create_receipt_handler_exception(self):
		...
		with patch("src.adapters.rest.fastapi_routes.SfsMdReceiptHandler") as mock_handler:
			mock_handler.return_value.get_or_create.side_effect = ValueError("Invalid receipt data")
